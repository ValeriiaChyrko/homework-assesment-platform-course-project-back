...
namespace HomeworkAssignment.Application.Implementations.ChapterRelated
{
    ...
    {
        private readonly ILogger<ChapterAttachmentService> _logger = logger ?? throw new ArgumentNullException(nameof(logger));
        ...
        public async Task<RespondAttachmentDto> CreateChapterAttachmentAsync(Guid courseId, Guid chapterId, RequestAttachmentDto attachmentDto,
            CancellationToken cancellationToken = default)
        {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            _logger.LogInformation("Successfully deleted chapter attachment with ATTACHMENT_ID: {AttachmentId}", attachmentId);
        }
    }
}