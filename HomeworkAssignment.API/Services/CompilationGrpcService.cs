...
public class CompilationGrpcService : ICompilationGrpcService
{
    ...
    public CompilationGrpcService(CompilationOperator.CompilationOperatorClient client, ILogger<CompilationGrpcService> logger)
    {
        _client = client;
        _logger = logger;
    }

    public async Task<int> VerifyProjectCompilation(RequestRepositoryWithBranchDto query, CancellationToken cancellationToken = default)
    {
        _logger.LogInformation("Verifying compilation for repo: {RepoTitle}, branch: {BranchTitle}, owner: {OwnerGithubUsername}, author: {AuthorGithubUsername}", 
        ...
        _logger.LogInformation("Received compilation score: {Score} for repo: {RepoTitle}, branch: {BranchTitle}.", response.Score, query.RepoTitle, query.BranchTitle);
