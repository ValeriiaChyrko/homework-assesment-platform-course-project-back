...
namespace HomeworkAssignment.Application.Implementations.ChapterRelated
{
    ...
    {
        private readonly ILogger<ChapterProgressService> _logger = logger ?? throw new ArgumentNullException(nameof(logger));
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            var userProgress = await ExecuteTransactionAsync(
                async () => await mediator.Send(new GetUserProgressByIdQuery(userId, chapterId), cancellationToken),
        ...
        private async Task<RespondChapterUserProgressDto> CreateUserProgressAsync(Guid userId, Guid chapterId, RequestChapterUserProgressDto chapterUserProgressDto, CancellationToken cancellationToken)
        {
        ...
        private async Task<RespondChapterUserProgressDto> UpdateUserProgressAsync(ChapterUserProgress userProgress, RequestChapterUserProgressDto chapterUserProgressDto, CancellationToken cancellationToken)
        {
            ...
            _logger.LogInformation("Successfully updated user progress with CHAPTER_ID: {ChapterId}", userProgress.ChapterId);
