...
[ApiController]
[Route("api/courses")]
public class CourseController(ICourseService service, HybridCache cache, ICacheKeyManager cacheKeyManager)
    : BaseController
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var cachedCourses = await cache.GetOrCreateAsync(
            ...
            _cacheOptions,
            [cacheKeyManager.CourseListGroup(userId)],
            cancellationToken);
    ...
    /// <summary>
    ///     Retrieves a single course based on filter parameters and studentId.
    /// </summary>
    [HttpGet("{courseId:guid}/progress")]
    public async Task<IActionResult> GetByStudent(Guid courseId,
        [FromQuery] RequestCourseFilterParameters filterParameters,
        [FromServices] IValidator<RequestCourseFilterParameters> validator,
        CancellationToken cancellationToken = default)
    {
        ...
        var userId = GetUserId();
        var cacheKey = cacheKeyManager.CourseSingleStudent(userId, courseId);
        var cachedCourse = await cache.GetOrCreateAsync(
            cacheKey,
            async _ => await service.GetSingleCourseFullInfoByStudentIdAsync(filterParameters, userId, courseId,
                cancellationToken),
            _cacheOptions,
            [cacheKeyManager.CourseSingleGroup(courseId)],
            cancellationToken);
